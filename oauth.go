...
func (c *Config) GetManagementToken(grantType OAuthGrantType) (authorizer autorest.Authorizer, err error) {
	...
	servicePrincipalToken, err := GetServicePrincipalToken(&c.AzureAuthConfig, env, rmEndPoint)
	if err != nil {
		return nil, err
	}
...
func (c *Config) GetKeyVaultToken(grantType OAuthGrantType) (authorizer autorest.Authorizer, err error) {
	env, err := ParseAzureEnvironment(c.Cloud)
	if err != nil {
		return nil, err
	}
	...
	servicePrincipalToken, err := GetServicePrincipalToken(&c.AzureAuthConfig, env, kvEndPoint)
	if err != nil {
		return nil, err
	}
	authorizer = autorest.NewBearerAuthorizer(servicePrincipalToken)
